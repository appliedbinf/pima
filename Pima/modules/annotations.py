...
def validate_features(pima_data: PimaData, settings: Settings):
    ...
    #validate ba_virulence_genes
    if not validate_file_and_size(pima_data, settings.ba_virulence_genes):
        pima_data.errors.append(
            f"Can't find Bacillus anthracis virulence gene database"
        )
...
def validate_blast(pima_data: PimaData, settings: Settings):
    ...
    pima_data.analysis.append(["blast_ba_virulence_genes", pima_data, settings])


def blast_feature_sets(pima_data: PimaData):
    """Find genes within both 'amr' and 'inc' databases within the assembly
    
    Generates a dictionary of dataframes, 1 dataframe for each feature file
    """
    ...
    if find_checkpoint(pima_data, pima_data.features_dir):
        ...
        found_feature_dirs = [
            ...
            if feature_dir.is_dir() and feature_dir.name in ["amr", "inc"]
        ]
    ...
    for feature_number in range(len(pima_data.feature_fastas)):
        ...
        blast_features(pima_data, feature_fasta, feature_dir, feature_name, 95.0)
...
def blast_ba_virulence_genes(pima_data: PimaData, settings: Settings):
    """Find Bacillus anthracis virulence genes within the genome assembly
    
    Generates a dataframe of the results
    """
    ...
    # Check if results already exist
    if find_checkpoint(pima_data, pima_data.features_dir):
        expected_result = os.path.join(pima_data.features_dir, 'ba_virulence_genes/best.bed')
        if os.path.exists(os.path.join(pima_data.features_dir, 'ba_virulence_genes')) and validate_file_and_size(pima_data, expected_result):
            print_and_log(
                pima_data,
                "Using previously identified virulence blast results",
                pima_data.sub_process_verbosity,
                pima_data.sub_process_color,
            )
            parse_blast_features(pima_data, expected_result, "ba_virulence_genes"), 
            return
        elif os.path.exists(os.path.join(pima_data.features_dir, 'ba_virulence_genes')):
            shutil.rmtree(os.path.join(pima_data.features_dir, 'ba_virulence_genes'))
    #Specifically look for all Ba virulence genes

    feature_dir = os.path.join(pima_data.features_dir, "ba_virulence_genes")
    os.makedirs(feature_dir)
    feature_fasta = settings.ba_virulence_genes
    
    blast_output = os.path.join(feature_dir, "blast_output.tsv")
    print_and_log(
        pima_data,
        "BLASTing virulence genes against the assembly",
        pima_data.sub_process_verbosity,
        pima_data.sub_process_color,
    )
    ...
    print_and_log(
        pima_data,
        "Converting virulence hits to BED",
        pima_data.sub_process_verbosity,
        pima_data.sub_process_color,
    )
    ...
    command = " ".join(
        [
            ...
            + 'print $2,($9 < $10 ? $9 : $10),($9 < $10 ? $10 : $9),$1,$3/100,($9 < $10 ? "+" : "-"),$4/$14}\'',
            "| sort -k 1,1 -k 2,2n >",
            all_bed,
        ]
    )
    ...
    command = " ".join(
        [
            ...
            "-f .9 -F .9 -wao",
            "| awk '$7 != \".\"'",
            '| awk \'{OFS="\\t";locus=$7"\\t"$8"\\t"$9; if($5 > s[locus]){s[locus]=$5;id = sprintf("%.3f", $5); b[locus] = $1"\\t"$2"\\t"$3"\\t"$4"\\t"id"\\t"$6"\\t"$7}}',
    ...
    print_and_run(pima_data, command)
    parse_blast_features(pima_data, best_bed, "ba_virulence_genes")
